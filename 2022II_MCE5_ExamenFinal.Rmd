...
Datos_puma <- filter(occ$data, country == 'Ecuador')
shp <- raster::getData('GADM', country = 'ECU', level = 1)

ggplot() +
  geom_point(data = Datos_puma, aes(x = decimalLongitude, y = decimalLatitude), color = 'black') +
  geom_polygon(data = shp, aes(x = long, y = lat, group = group), color = 'grey', fill = NA) +
  coord_fixed(ylim = c(-5, 2.5), xlim = c(-92.5, -30)) +
  xlab('Longitud') +
  ylab('Latitud')

```
**descripcion** Se utilizaron las librerias rgbif y tidyverse para obtener datos de ocurrencia de la especie "Puma concolor" en Ecuador utilizando de GBIF. Luego, se filtran los registros para seleccionar con coordenadas geográficas válidas y se visualizan en un gráfico de puntos sobre un mapa de Ecuador. El objetivo es identificar las ubicaciones donde se han registrado avistamientos de pumas en el país y evaluar su distribución geográfica.
...
Datos_ceiba <- filter(occ$data, country == 'Ecuador')

shp <- raster::getData('GADM', country = 'ECU', level = 1)

ggplot() +
  geom_point(data = Datos_ceiba, aes(x = decimalLongitude, y = decimalLatitude), color = 'black') +
  geom_polygon(data = shp, aes(x = long, y = lat, group = group), color = 'grey', fill = NA) +
  coord_fixed(ylim = c(-5, 2.5), xlim = c(-92.5, -30)) +
  xlab('Longitud') +
  ylab('Latitud')
...
```
**descripcion**  Se obtuvo datos de la ocurrencia de la especie Ceiba pentandra en el Ecuador, utilizando GBIF, se filtraron los registros para la seleccion con las coordenadas válidas y finalmente se logra vizualizar la distribución de la sp, dentro del mapa.
